...
export function AllDayRow({ weekDays }: AllDayRowProps) {
  ...
  if (allDayEvents.length === 0) {
    return (
      <div className="flex min-h-cal-allday shrink-0 border-b border-cal-grid-line">
        <div className="w-cal-gutter shrink-0 border-r border-cal-grid-line" />
        <div className="flex flex-1">
          {weekDays.map((day) => (
            <div
              key={day.toISOString()}
              className="min-w-0 flex-1 border-r border-cal-grid-line last:border-r-0"
            />
          ))}
        </div>
      </div>
  ...
  return (
    <div className="flex min-h-cal-allday shrink-0 border-b border-cal-grid-line">
      <div className="flex w-cal-gutter shrink-0 items-start justify-end border-r border-cal-grid-line pr-2 pt-1">
      ...
      <div className="relative flex flex-1">
        {weekDays.map((day) => (
          <div
            key={day.toISOString()}
            className="min-w-0 flex-1 border-r border-cal-grid-line p-0.5 last:border-r-0"
          >
            {allDayEvents
